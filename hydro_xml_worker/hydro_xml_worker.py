...
async def download_hydro_data():
    ...
    async def debug_screenshot(page, label=""):
        ...
        try:
            ...
            name = os.path.join(DOWNLOAD_DIR, f"debug_{safe_label}.png")
    ...
    try:
        ...
        logger.debug(f"URL after initial navigation: {page.url}")
        ...
        logger.debug(f"URL when login form is visible: {page.url}")

        # 3. Enter Credentials and Login

        logger.debug("Entering credentials...")
        await page.evaluate(LOGIN_JS % (user_email, user_pass))
        ...
        logger.debug(f"URL after attempting to click DownloadMyData: {page.url}")
        ...
        logger.debug(f"URL on download page: {page.url}")
        ...
        await page.evaluate(EXPORT_PREP_JS, from_date_str)
    ...
    while True:
        try:
            ...
            try:
                ...
                logger.error(
                    "Scrape timed out after 300 seconds; will retry next interval."
                )
